...
void OpenGLWidget::drawcone(double r, double h)
{
    ...
    for (int i = 0; i <360; i += lsrule.cylinder_step) {
            //draw a sector
            float temp = i * Pi / 180;
            float temp1 = (i + lsrule.cylinder_step)*Pi / 180;

            //glBindTexture(GL_TEXTURE_2D, texture[1]);
            glBegin(GL_QUADS);//画四边形
            glColor3ub(107, 142, 35);//选择颜色
            //glTexCoord2f(0.0 , 0.0);
            glVertex3f(r*cos(temp)*grape.trunk.radius_shrink, 0, r*sin(temp)*grape.trunk.radius_shrink);//起点端点
            glColor3ub(107, 142, 35);
            //glTexCoord2f(1.0 , 0.0);
            glVertex3f(r*cos(temp), h, r*sin(temp));//终点端点
...
void OpenGLWidget::DrawChannel(Node A, Node B, double r)
{
    GLfloat   dx = B.x - A.x;
        ...
        GLfloat   distance = sqrt(dx*dx + dy * dy + dz * dz);//终点理起点的距离
        ...
        glRotatef(angle, fx, fy, fz);//绕法向量旋转
...
void OpenGLWidget::drawSphere(double r) {
    int i, j;
    for(i = 0; i <= lsrule.lats; i++) {
        //Draw a stack
        double lat0 = M_PI * (-0.5 + (double) (i - 1) / lsrule.lats);
        double z0  = sin(lat0);
        double zr0 =  cos(lat0);

        double lat1 = M_PI * (-0.5 + (double) i / lsrule.lats);
        double z1 = sin(lat1);
        double zr1 = cos(lat1);

        //glBindTexture(GL_TEXTURE_2D, texture[0]);
        glBegin(GL_QUAD_STRIP);
        for(j = 0; j <= lsrule.longs; j++) {
            //Draw a sector
            double lng = 2 * M_PI * (double) (j - 1) / lsrule.longs;
            double x = cos(lng);
            double y = sin(lng);

//            glColor3ub(130, 42, 58);//table grape
//           glColor3ub(8,46,84);//wine grape
                        glColor3ub(color_a,color_b,color_c);//wine grape
